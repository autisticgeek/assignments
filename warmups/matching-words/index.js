...
const matchWords = str => {
    ...
    str.replace(/[^a-z\s]/gi, "").toLowerCase().split(" ").forEach(str => { //replace that wich is not a letter or a space, convert to all lowercaase, devide it into an array, fun a function on eachb part of the array
        if (counts.hasOwnProperty(str)) { //check to see if we have seen this word before
            if (!output.includes(str)) { //check to see of word is NOT already in the output array
                output.push(str);// adds word to the output array 
            }
        } else {
            counts[str] = true; //adds word to counts object
        }
    })
